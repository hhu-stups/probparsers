...
public class ExpressionTest {
	...
	public void testQuantifiedUnionExpression2() throws Exception {
		...
		final String result = getTreeAsString(testMachine);
		assertEquals(
				"Start(AExpressionParseUnit(AQuantifiedUnionExpression([AIdentifierExpression([x]),AIdentifierExpression([y])],AConjdisjunctPredicate(AElementconjdisjunctPredicate(AEqualPredicate(AIdentifierExpression([x]),AIntegerExpression(0)),&),AEqualPredicate(AIdentifierExpression([y]),AIdentifierExpression([x]))),ACoupleExpression([AIdentifierExpression([x]),AIdentifierExpression([y])]))))",
				result);
	}
