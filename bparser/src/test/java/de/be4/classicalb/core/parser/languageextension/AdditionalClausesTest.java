...
public class AdditionalClausesTest {
	...
	public void testExpressionsClause() throws Exception {
		...
		assertTrue(result.contains(
				"[expressions(none,[expression_definition(none,foo,[identifier(none,a)],integer(none,1)),expression_definition(none,bar,[],boolean_true(none))])]"));
	}
	...
	public void testPredicatesClause() throws Exception {
		...
		final String result = Helpers.getMachineAsPrologTerm(testMachine);
		assertTrue(result.contains(
				"[predicates(none,[predicate_definition(none,foo,[identifier(none,a)],equal(none,integer(none,1),integer(none,1))),predicate_definition(none,bar,[],equal(none,integer(none,2),integer(none,2)))])]"));
	}
}
