...
public class RealOrSetOfPairType extends AbstractHasFollowers {
	...
	public RealOrSetOfPairType(SourcePosition sourcePosition,
			SourcePosition sourcePosition2) {

		RealOrSetType i1 = new RealOrSetType();
		this.first = i1;
		first.addFollower(this);

		RealOrSetType i2 = new RealOrSetType();
		this.second = i2;
	...
	public void update(BType oldType, BType newType, ITypechecker typechecker) {
		...
		if (newType instanceof RealType) {
			// if newType is an Real then both arguments and the result are
			// Reals
			...
			return;
		} else if (newType instanceof RealOrSetOfPairType) {
			if (this.first == oldType) {
				first.deleteFollower(this);
		...
		} else if (newType instanceof RealOrSetType) {
			if (this.first == oldType) {
				first.deleteFollower(this);
				first = (AbstractHasFollowers) newType;
				first.addFollower(this);
	...
	public BType unify(BType other, ITypechecker typechecker) {
		...
		if (other instanceof RealType) {
			...
			first.unify(RealType.getInstance(), typechecker);
			second.unify(RealType.getInstance(), typechecker);
		...
		if(other instanceof RealOrSetOfPairType){
			RealOrSetOfPairType o = (RealOrSetOfPairType) other;
			o.first.deleteFollower(o);
			o.second.deleteFollower(o);
			first = (AbstractHasFollowers) this.first.unify(((RealOrSetOfPairType) other).first, typechecker);
			second = (AbstractHasFollowers) this.second.unify(((RealOrSetOfPairType) other).second, typechecker);
			((RealOrSetOfPairType) other).setFollowersTo(this, typechecker);
			return this;
		}
		throw new RuntimeException();
	}
	...
	public String toString() {
		return "RealOrSetOfPairType" + this.hashCode() + "("
