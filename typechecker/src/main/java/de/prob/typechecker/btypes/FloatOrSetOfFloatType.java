...
public class FloatOrSetOfPairType extends AbstractHasFollowers {
	...
	public FloatOrSetOfPairType(SourcePosition sourcePosition,
			SourcePosition sourcePosition2) {

		FloatOrSetType i1 = new FloatOrSetType();
		this.first = i1;
		first.addFollower(this);

		FloatOrSetType i2 = new FloatOrSetType();
		this.second = i2;
	...
	public void update(BType oldType, BType newType, ITypechecker typechecker) {
		...
		if (newType instanceof FloatType) {
			// if newType is an Float then both arguments and the result are
			// Floats
			...
			this.setFollowersTo(FloatType.getInstance(), typechecker);
			...
			return;
		} else if (newType instanceof FloatOrSetOfPairType) {
			if (this.first == oldType) {
				first.deleteFollower(this);
		...
		} else if (newType instanceof FloatOrSetType) {
			if (this.first == oldType) {
				first.deleteFollower(this);
				first = (AbstractHasFollowers) newType;
				first.addFollower(this);
	...
	public BType unify(BType other, ITypechecker typechecker) {
		...
		if (other instanceof FloatType) {
			...
			first.unify(FloatType.getInstance(), typechecker);
			second.unify(FloatType.getInstance(), typechecker);
			return FloatType.getInstance();
		}
		
		if (other instanceof FloatOrSetType) {
			((FloatOrSetType) other).setFollowersTo(this, typechecker);
			return this;
		}
		...
		if(other instanceof FloatOrSetOfPairType){
			FloatOrSetOfPairType o = (FloatOrSetOfPairType) other;
			o.first.deleteFollower(o);
			o.second.deleteFollower(o);
			first = (AbstractHasFollowers) this.first.unify(((FloatOrSetOfPairType) other).first, typechecker);
			second = (AbstractHasFollowers) this.second.unify(((FloatOrSetOfPairType) other).second, typechecker);
			((FloatOrSetOfPairType) other).setFollowersTo(this, typechecker);
			return this;
		}
		throw new RuntimeException();
	}
	...
	public String toString() {
		return "FloatOrSetOfPairType" + this.hashCode() + "("
	...
	public boolean compare(BType other) {
		if (other instanceof UntypedType || other instanceof FloatType
				|| other instanceof FloatOrSetType
